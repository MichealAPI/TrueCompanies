...
public final class SignMenuAPI {
    ...
    @Builder
    public SignMenuAPI(SignCompleteHandler action, List<String> withLines, UUID uuid, Plugin plugin) {
    ...
    private class LeaveListener implements Listener {
        ...
        public void onLeave(PlayerQuitEvent e) {
            if (e.getPlayer().getUniqueId().equals(SignMenuAPI.this.uuid)) {
                ProtocolLibrary.getProtocolManager().removePacketListener(SignMenuAPI.this.packetListener);
                HandlerList.unregisterAll(this);
                SignMenuAPI.this.sign.getBlock().setType(Material.AIR);
            }
        }
    }
    ...
    private void registerSignUpdateListener() {
        ...
        this.packetListener = new PacketAdapter(plugin, PacketType.Play.Client.UPDATE_SIGN) {
            public void onPacketReceiving(PacketEvent event) {
                if (event.getPlayer().getUniqueId().equals(SignMenuAPI.this.uuid)) {
                    List<String> lines = Stream.of(0,1,2,3).map(line -> getLine(event, line)).collect(Collectors.toList());

                    Bukkit.getScheduler().runTask(plugin, () -> {
                        manager.removePacketListener(this);

                        HandlerList.unregisterAll(SignMenuAPI.this.listener);

                        SignMenuAPI.this.sign.getBlock().setType(Material.AIR);

                        SignMenuAPI.this.action.onSignClose(new SignCompletedEvent(event.getPlayer(), lines));
                    });
                }
            }
        };
